...
def test_wan_client_retry_503(mock_sleep, mock_post, tmp_path):
    # First response fails with 503, second succeeds
...
def test_composer_combine(mock_run, tmp_path):
    # Mock subprocess to avoid running ffmpeg
    ...
    # We should have two subprocess calls (one for txt concat, one for final encode)
